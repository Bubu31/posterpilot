...
- **缩略图缓存**（`THUMB_CACHE_TTL_DAYS`，默认 `30`；`THUMB_CACHE_MAX_MB`，
  ...
  逐出。
- **库默认排序**（`LIBRARY_DEFAULT_SORT`，默认 `title`）。当 URL 未指定
  排序时，库墙打开时使用的排序：`title`、`year`、`rating`、`runtime`、
  `recent`（最近变更）或 `added`（媒体服务器上的添加日期）。在库工具栏中
  选择排序始终会覆盖它。

## FUN 板块

**FUN**（`FUN_ENABLED`，默认关闭）是一个可自愿启用的媒体库实验板块，
在你于 **设置 → Kometa 与高级** 中启用它之前保持隐藏。它的第一个工具
是一个随机电影/剧集抽选器——参见
[使用](/posterpilot/zh/usage/#fun随机电影剧集抽选器)。
当开关关闭时，FUN 没有导航入口，其页面会返回 404。
...
| `LIBRARY_DEFAULT_SORT`    | 库默认排序                | `title`                               | 库墙打开时使用的排序：`title`、`year`、`rating`、`runtime`、`recent` 或 `added`。             |
| `FUN_ENABLED`             | FUN 板块                  | off                                   | 显示实验性的 FUN 板块（随机电影/剧集抽选器）。                                                 |
